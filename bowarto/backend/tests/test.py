...
from bowarto.backend.tests.auth import login, refresh
...
def test_user_create_competition():
...
def test_user_create_application():
    access_token, refresh_token = login("user_0@example.com", '123')
...
def test_user_get_applications():
    access_token, refresh_token = login("user_0@example.com", '123')
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json"
...
def test_user_one_application_created_by_another_user():
    access_token, refresh_token = login("user_0@example.com", '123')
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json"
...
if __name__ == '__main__':
    ...
    test_user_create_application()
    # test_user_get_applications()
    # test_user_one_application()
    test_user_one_application_created_by_another_user()
